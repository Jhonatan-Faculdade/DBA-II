...
class Curso(BaseModel):
    ...
    def validar_titulo(cls, value: str):
        ...
        if len(palavras) < 2:
            raise ValueError('Informe pelo menos duas palavras tongo véio!')
        
        if value.islower():
            raise ValueError('O bocó alguma coisa em maiusculo voce digitar ter')
    ...
    def validar_horas(cls, value: int): #maior que 30
        if (value < 30):
            raise ValueError('Tem que ter 30 ou mais aulas')
...
cursos = [
    ...
    Curso(id=4, nome="Programacao 4 monster", aulas=20, horas=30),
    Curso(id=5, nome="Programacao 5 ultra", aulas=30, horas=333),
    Curso(id=6, nome="Manutencao de ar condicionado", aulas=100, 
          horas=250),
    Curso(id=7, nome="Atualizacai de impressora", aulas=100, 
          horas=250),
    Curso(id=8, nome="Algoritmos 1", aulas=100, 
          horas=250),
